...
class Service(Base):
    ...
    id, name, price, unit = Column(Integer, primary_key=True), Column(String(200), nullable=False), Column(Float, default=0.0), Column(String(50), default="шт")
...
class Deal(Base):
    __tablename__ = "deals"
    id, contact_id, stage_id, title = Column(Integer, primary_key=True), Column(Integer, ForeignKey("contacts.id")), Column(Integer, ForeignKey("stages.id")), Column(String(200), nullable=False)
...
class Equipment(Base):
    ...
    id, name, model, serial = Column(Integer, primary_key=True), Column(String(200), nullable=False), Column(String(200), default=""), Column(String(100))
    purchase_date, purchase_cost, engine_hours = Column(Date), Column(Float, default=0.0), Column(Float, default=0.0)
    status, notes = Column(String(50), default="active"), Column(Text)
...
class Expense(Base):
    ...
    category_id, equipment_id = Column(Integer, ForeignKey("expense_categories.id")), Column(Integer, ForeignKey("equipment.id"))
...
class Consumable(Base):
    ...
    id, name, unit, stock_quantity, notes = Column(Integer, primary_key=True), Column(String(200), nullable=False, unique=True), Column(String(50), default="шт"), Column(Float, default=0.0), Column(Text)
        ...
        with SessionFactory() as s:
            if s.query(Stage).count() == 0:
                s.add_all([Stage(**d) for d in [
                    ...
                    {"name": "В работе", "order": 3, "color": "#EC4899"}, {"name": "Успешно", "order": 4, "color": "#10B981", "is_final": True},
            ...
            if s.query(Service).count() == 0:
                s.add_all([Service(**d) for d in [
                    {"name":"Покос бурьяна/высокой травы","price":1500,"unit":"сотка"},
                    {"name":"Покос газона","price":800,"unit":"сотка"},
                    {"name":"Стрижка кустарника","price":500,"unit":"м.п."},
                    {"name":"Вспашка земли мотоблоком","price":2000,"unit":"сотка"},
                    {"name":"Спил дерева","price":1500,"unit":"шт"},
                    {"name":"Вывоз мусора","price":3000,"unit":"рейс"}
                ]])
            s.commit()
...
def get_tasks(year: Optional[int] = None, is_done: Optional[bool] = None, db: DBSession = Depends(get_db), _=Depends(get_current_user)):
    ...
    tasks_list = [{"id": t.id, "title": t.title, "description": t.description, "status": t.status, "is_done": t.is_done, 
                   "due_date": t.due_date.isoformat() if t.due_date else None, "assignee": t.assignee, 
                   "assignee_name": users.get(t.assignee, "Не назначен"), "priority": t.priority} for t in q.all()]
