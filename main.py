...
# ── 3.5. PYDANTIC-СХЕМЫ ДЛЯ ВАЛИДАЦИИ И СЕРИАЛИЗАЦИИ ──────────────────────────

# Используем BaseModel для всех схем, чтобы FastAPI мог корректно
# преобразовывать их в JSON и обратно.

class StageSchema(BaseModel):
    id: int; name: str; order: int; type: str; is_final: bool; color: str

class ContactSchema(BaseModel):
    id: int; name: str; phone: Optional[str]; source: Optional[str]

class ServiceSchema(BaseModel):
    id: int; name: str; price: float; unit: str; min_volume: float; notes: Optional[str]

class EquipmentSchema(BaseModel):
    id: int; name: str; model: Optional[str]; status: str; purchase_date: Optional[date]; purchase_cost: Optional[float]
    engine_hours: Optional[float]; fuel_norm: Optional[float]; last_maintenance_date: Optional[date]; next_maintenance_date: Optional[date]

class ConsumableSchema(BaseModel):
    id: int; name: str; unit: str; stock_quantity: float; price: float; notes: Optional[str]

class ExpenseCategorySchema(BaseModel):
    id: int; name: str

class ExpenseSchema(BaseModel):
    id: int; name: str; amount: float; date: date; category: Optional[str]

class TaskSchema(BaseModel):
    id: int; title: str; description: Optional[str]; is_done: bool; due_date: Optional[date]
    priority: str; status: str; assignee: Optional[str]; assignee_name: str

class TaxPaymentSchema(BaseModel):
    id: int; amount: float; date: date; note: Optional[str]; year: int
...
def decode_access_token(token: str) -> dict:
    try:
        ...
        if not key: raise JWTError("Signing key not found")
        return jwt.decode(token, key, algorithms=["RS256"], audience=AUTH0_AUDIENCE, issuer=f"https://{AUTH0_DOMAIN}/")
    except Exception as e: raise JWTError(f"Token decoding failed: {e}")
...
def login(request: Request):
    state = secrets.token_urlsafe(16); request.session["oauth_state"] = state
...
def callback(request: Request, code: str = None, state: str = None, error: str = None):
    ...
    if not code or state != request.session.pop("oauth_state", None): raise HTTPException(400, "Invalid state or no code")
    ...
    request.session["user"] = {"sub": payload.get("sub", ""), "role": payload.get(ROLE_CLAIM, "user")}; return RedirectResponse("/")

@app.get("/api/auth/logout", include_in_schema=False)
def logout(request: Request):
    request.session.clear(); return RedirectResponse(f"https://{AUTH0_DOMAIN}/v2/logout?client_id={CLIENT_ID}&returnTo={APP_BASE_URL}")

# ── 8. ОСНОВНЫЕ DATA ЭНДПОИНТЫ (CRUD) ──────────────────────────────────────────

class User(BaseModel): id: str; name: str

@app.get("/api/users", response_model=List[User])
def get_users(_=Depends(get_current_user)):
    # Заглушка, в реальном приложении здесь будет запрос к таблице пользователей
...
def get_years(db: DBSession = Depends(get_db), _=Depends(get_current_user)):
    ...
    return sorted(list(years)) if years else [datetime.utcnow().year]

# --- КОРРЕКТНЫЙ СПОСОБ: ВОЗВРАЩАЕМ СПИСОК СЛОВАРЕЙ, А НЕ ОБЪЕКТЫ БАЗЫ --- 

@app.get("/api/stages", response_model=List[StageSchema])
def get_stages(db: DBSession = Depends(get_db), _=Depends(get_current_user)):
    return [StageSchema.from_orm(s) for s in db.query(Stage).order_by(Stage.order).all()]

# --- СДЕЛКИ (DEALS) --- 
class ServiceInDealSchema(BaseModel): service_id: int; quantity: float
class DealCreateUpdateSchema(BaseModel):
    title: str; contact_id: Optional[int] = None; new_contact_name: Optional[str] = None; stage_id: int
    manager: Optional[str] = None; services: List[ServiceInDealSchema] = []; discount: float = 0; tax_rate: float = 0; tax_included: bool = False
...
def _calculate_total(db: DBSession, services: List[ServiceInDealSchema], discount: float, tax_rate: float, tax_included: bool) -> float:
    subtotal = sum(db.query(Service).get(s.service_id).price * s.quantity for s in services)
    discounted_total = subtotal * (1 - discount / 100)
    return round(discounted_total if tax_included else discounted_total * (1 + tax_rate / 100), 2)
...
def create_deal(body: DealCreateUpdateSchema, db: DBSession = Depends(get_db), _=Depends(get_current_user)):
    ...
    if not contact_id and body.new_contact_name:
        ...
        db.add(contact); db.commit(); db.refresh(contact); contact_id = contact.id

    deal = Deal(title=body.title, contact_id=contact_id, stage_id=body.stage_id, manager=body.manager, discount=body.discount, tax_rate=body.tax_rate, tax_included=body.tax_included, deal_date=datetime.utcnow())
    ...
    db.add(deal); db.commit(); db.refresh(deal); _cache.invalidate("all"); return {"id": deal.id}
...
def update_deal(deal_id: int, body: DealCreateUpdateSchema, db: DBSession = Depends(get_db), _=Depends(get_current_user)):
    ...
    for key, value in body.dict(exclude={'services','new_contact_name'}).items(): setattr(deal, key, value)
    deal.total = _calculate_total(db, body.services, body.discount, body.tax_rate, body.tax_included)
    deal.service_links.clear()
    for s in body.services: deal.service_links.append(DealService(service_id=s.service_id, quantity=s.quantity, price_at_moment=db.query(Service).get(s.service_id).price))
    db.commit(); _cache.invalidate("all"); return {"ok": True}
...
def get_deal_details(deal_id: int, db: DBSession = Depends(get_db), _=Depends(get_current_user)):
    deal = db.query(Deal).get(deal_id)
    if not deal: raise HTTPException(404, "Deal not found")
    return {"id": deal.id, "title": deal.title, "total": deal.total, "stage_id": deal.stage_id,
            ...
            "services": [{"service": {"id": sl.service.id, "name": sl.service.name, "unit": sl.service.unit, "min_volume": sl.service.min_volume}, "quantity": sl.quantity, "price_at_moment": sl.price_at_moment} for sl in deal.service_links]}
...
def update_deal_stage(deal_id: int, body: dict, db: DBSession = Depends(get_db), _=Depends(get_current_user)):
    ...
    if not deal or not stage: raise HTTPException(404, "Deal or Stage not found")
    deal.stage_id = stage.id; db.commit(); _cache.invalidate("all"); return {"ok": True}

# --- ЗАДАЧИ, РАСХОДЫ И ДРУГИЕ CRUD --- 
...
def get_tasks(year: Optional[int] = None, is_done: Optional[bool] = None, db: DBSession = Depends(get_db), _=Depends(get_current_user)):
    ...
    return [TaskSchema(**t.__dict__, assignee_name=users.get(t.assignee, '-')) for t in q.all()]

class TaskCreate(BaseModel): title: str; description: Optional[str]=None; due_date: Optional[date]=None; is_done: bool=False; priority:str='Обычный'; assignee:Optional[str]=None; status:str='Открыта'

@app.post("/api/tasks", status_code=201)
def create_task(body: TaskCreate, db: DBSession = Depends(get_db), _=Depends(get_current_user)):
    task = Task(**body.dict()); db.add(task); db.commit(); db.refresh(task); _cache.invalidate("tasks"); return {"id": task.id}
...
def update_task(task_id: int, body: TaskCreate, db: DBSession = Depends(get_db), _=Depends(get_current_user)):
    ...
    db.commit(); _cache.invalidate("tasks"); return {"id": task.id}
...
def get_expenses(year: Optional[int] = None, db: DBSession = Depends(get_db), _=Depends(get_current_user)):
    ...
    return [ExpenseSchema(id=e.id, name=e.name, amount=e.amount, date=e.date, category=e.category.name if e.category else None) for e in q.all()]

class ExpenseCreate(BaseModel): name: str; amount: float; date: date; category: Optional[str] = None

@app.post("/api/expenses", status_code=201)
def create_expense(body: ExpenseCreate, db: DBSession = Depends(get_db), _=Depends(get_current_user)):
    cat = db.query(ExpenseCategory).filter_by(name=body.category).first() if body.category else None
    if body.category and not cat: cat = ExpenseCategory(name=body.category); db.add(cat); db.commit(); db.refresh(cat)
    exp = Expense(name=body.name, amount=body.amount, date=body.date, category_id=cat.id if cat else None)
    db.add(exp); db.commit(); db.refresh(exp); _cache.invalidate("all"); return {"id": exp.id}
...
def update_expense(expense_id: int, body: ExpenseCreate, db: DBSession = Depends(get_db), _=Depends(get_current_user)):
    ...
    cat = db.query(ExpenseCategory).filter_by(name=body.category).first() if body.category else None
    if body.category and not cat: cat = ExpenseCategory(name=body.category); db.add(cat); db.commit(); db.refresh(cat)
    exp.name=body.name; exp.amount=body.amount; exp.date=body.date; exp.category_id=cat.id if cat else None
    db.commit(); _cache.invalidate("all"); return {"id": exp.id}
...
def get_expense_categories(db: DBSession = Depends(get_db), _=Depends(get_current_user)):
    return [ExpenseCategorySchema.from_orm(c) for c in db.query(ExpenseCategory).all()]
...
def get_equipment(db: DBSession = Depends(get_db), _=Depends(get_current_user)):
    return [EquipmentSchema.from_orm(e) for e in db.query(Equipment).order_by(Equipment.name).all()]
...
def get_services(db: DBSession = Depends(get_db), _=Depends(get_current_user)):
    return [ServiceSchema.from_orm(s) for s in db.query(Service).order_by(Service.name).all()]
...
def get_consumables(db: DBSession = Depends(get_db), _=Depends(get_current_user)):
    return [ConsumableSchema.from_orm(c) for c in db.query(Consumable).order_by(Consumable.name).all()]
...
def get_contacts(db: DBSession = Depends(get_db), _=Depends(get_current_user)):
    return [ContactSchema.from_orm(c) for c in db.query(Contact).order_by(Contact.name).all()]

class ContactCreate(BaseModel): name: str; phone: Optional[str] = None; source: Optional[str] = None

@app.post("/api/contacts", status_code=201, response_model=ContactSchema)
def create_contact(body: ContactCreate, db: DBSession = Depends(get_db), _=Depends(get_current_user)):
    contact = Contact(**body.dict()); db.add(contact); db.commit(); db.refresh(contact); _cache.invalidate("contacts"); return contact
...
def get_tax_payments(year: int, db: DBSession = Depends(get_db), _=Depends(get_current_user)):
    return [TaxPaymentSchema.from_orm(p) for p in db.query(TaxPayment).filter(TaxPayment.year == year).order_by(TaxPayment.date.desc()).all()]
...
def create_tax_payment(body: TaxPaymentCreate, db: DBSession = Depends(get_db), _=Depends(get_current_user)):
    ...
    _cache.invalidate(f"taxes_summary:{body.year}", f"taxes_payments:{body.year}"); return payment
